...
function ImageUpload({ setResults }) {
  ...
  const originalFileName = image.name.replace(/\s+/g, '_'); // Preserve original filename (remove spaces)
  const imageFile = new File([croppedBlob], originalFileName, { type: image.type });
  ...
  try {
    ...
    if (response.data && response.data.results) {
      ...
      let text = "Search successful! Images from " + response.data.worker;
...
const uploadImages = async () => {
  ...
  setUploadStatus("Uploading filenames...");

  // Extract filenames only
  const filenames = selectedImages.map((image) => image.name.replace(/\s+/g, '_')); // Replace spaces with underscores

  try {
    const response = await axios.post(
      serverurl + "/insert_images",
      { filenames }, // Send filenames as JSON
      { headers: { "Content-Type": "application/json" } } // Ensure JSON format
    );

    console.log("Uploaded Filenames Response:", response.data);
